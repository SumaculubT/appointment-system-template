...
const ManageServices = () => {
  const { services, fetchServices } = useStateContext();
  const [loading, setLoading] = useState(false);

  const [selectedServiceNav, setSelectedServiceNav] = useState("all");
  ...
  useEffect(() => {
    getServices(1, selectedServiceNav, searchTerm);
  }, [selectedServiceNav, searchTerm]);
  ...
  const serviceNav = ["all", "electronics", "clothing", "furniture", "books"];

  const filteredServices =
    selectedServiceNav !== "all"
      ? services.filter((s) => s.category === selectedServiceNav.toLowerCase())
      : services;
            ...
            <div className=" flex flex-row gap-6">
              {serviceNav.map((category) => (
                <a
                  key={category}
                  onClick={() => {
                    setSelectedServiceNav(category);
                  }}
                  className={`cursor-pointer px-2 pb-2 hover:text-blue-500 ${
                    selectedServiceNav === category
              ...
              {pagination?.meta?.current_page > 2 && (
                <li>
                  <button
                    ...
                    onClick={() => {
                      getServices(1, selectedServiceNav);
                    }}
                  >
                    {"< First"}
                  </button>
              ...
              {pagination?.meta?.current_page > 1 && (
                <li>
                  <button
                    className="cursor-pointer text-sm text-gray-700 font-semibold px-1 py-2 hover:bg-gray-100 rounded-sm duration-100"
                    onClick={() =>
                      getServices(currentPage - 1, selectedServiceNav)
                    }
                  >
                    Previous
                  </button>
              ...
              {pagination?.meta?.current_page !==
                ...
                <li>
                  <button
                    ...
                    onClick={() =>
                      getServices(pagination.meta.last_page, selectedServiceNav)
                    }
                  >
                    {"Last >"}
                  </button>
